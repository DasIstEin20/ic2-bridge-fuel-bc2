...
public class EnergyNetwork extends DynamicBufferedNetwork<IStrictEnergyHandler, EnergyNetwork, FloatingLong, UniversalCable> implements IMekanismStrictEnergyHandler {
    ...
    /**
     * Simulates how much energy the currently connected acceptors can consume.
     *
     * <p>This is a read-only view of the same acceptor cache used by
     * {@link #tickEmit(FloatingLong)}. The IC2 adapter uses it to apply
     * backpressure before EU is converted and inserted into the cable buffer;
     * it does not alter Mekanism's distribution or buffering behaviour.</p>
     */
    @NotNull
    public FloatingLong getSimulatedAcceptorDemand(@NotNull FloatingLong maximum) {
        if (maximum.isZero()) {
            return FloatingLong.ZERO;
        }
        FloatingLong demand = FloatingLong.ZERO;
        for (Map<Direction, LazyOptional<IStrictEnergyHandler>> acceptors : acceptorCache.getAcceptorValues()) {
            for (LazyOptional<IStrictEnergyHandler> lazyAcceptor : acceptors.values()) {
                IStrictEnergyHandler acceptor = lazyAcceptor.resolve().orElse(null);
                if (acceptor == null) {
                    continue;
                }
                FloatingLong remaining = maximum.subtract(demand);
                if (remaining.isZero()) {
                    return maximum;
                }
                FloatingLong rejected = acceptor.insertEnergy(remaining, Action.SIMULATE);
                if (rejected.smallerThan(remaining)) {
                    demand = demand.add(remaining.subtract(rejected));
                }
            }
        }
        return demand.min(maximum);
    }
